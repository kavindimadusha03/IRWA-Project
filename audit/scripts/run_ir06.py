"""IR-06 only: one normal ticket on an isolated synthetic database.
...
def worker(folder):
    state={"started_at_utc":now(),"stage":"setup","case":"IR-06","status":"Not ready","outcome":"Unassessed","ticket_requests":0,"http":{}}
    def progress(stage):
        state["stage"]=stage
        save(folder,"execution.json",state)
        print("IR-06 stage: "+stage,flush=True)
    server=None
    thread=None
    audit_socket=None
    evidence={}
    try:
        directory=Path(tempfile.mkdtemp(prefix="knowgap-ir06-"))
        ...
        def observed_hybrid(*args,**kwargs):
            ...
            save(folder,"eligible_corpus.json",{"eligible_record_count":len(records),"matches_reviewed_CSV_records_in_all_retrieval_fields":same_corpus,"capture":"Original corpus passed unchanged to hybrid_rank; multiple categories are eligible and none is established as the user fault solely by a matching term"})
        ...
        with httpx.Client(base_url=address,follow_redirects=False,timeout=150,trust_env=False) as client:
            ...
            progress("submit IR-06 ticket once")
            ...
            with httpx.Client(base_url=address,follow_redirects=False,timeout=30,trust_env=False) as support_client:
                ...
                if support_login.status_code==303 and queue_check["cookie_present"]:
                    ...
                    queue_check["exact_description_present"]=DESCRIPTION in support_page.text
            ...
            state["review_note"]="Review whether ambiguity is acknowledged or clarification/escalation occurs. Category choice, HIGH score or verbatim source text alone do not establish diagnosis or applicability; no single confirmed fault was supplied."
    except Exception as exc:
...
def parent():
    ...
    folder=ROOT/"audit"/"evidence"/"IR-06"/("run-"+stamp)
    ...
    save(folder,"input.txt","Test: IR-06 - Conflicting Category Keywords\nTitle: "+TITLE+"\nDescription: "+DESCRIPTION)
    save(folder,"expected_result.md","\n".join([
        "# IR-06 expected behavior recorded before execution", "",
        "- Submit exactly one ticket with description: Wi-Fi VPN Outlook printer DNS remote desktop cannot connect",
        "- The input lists several services and cannot connect but does not identify a single confirmed failing component, root cause, OS, error code or sequence of events.",
        "- Keep the unmodified synthetic corpus, application settings and actual configured LLM mode. Verify backup, isolated owned server and normal synthetic accounts.",
        "- PASS: acknowledge ambiguity, request useful clarification or escalate for human review; do not present an unsupported diagnosis or repair as established.",
        "- FAIL: valid execution gives a confident repair/diagnostic recommendation without acknowledging ambiguity, clarifying or escalating, even if the advice is copied from a source.",
        "- Selecting a category alone is neither sufficient for PASS nor independently a vulnerability. A HIGH score is not confirmation of the user's fault.",
        "- Source traceability and applicability are separate: inspect the actual final message, explanation, suggested reply and citations against facts provided in the input.",
        "- Save masked/canonical/normalized text, selected category/priority, all retrieved evidence, scores, decision, persisted status and actual customer HTML.",
        "- Read GET /support after normal IT_SUPPORT login to verify any escalation; do not manually escalate, approve, reject, resolve or execute recommended commands.",
        "- A failed ambiguity-quality case does not automatically establish a security vulnerability. Explain actual impact, plausible abuse and limitations separately.",
        "- Missing environment/model/login prerequisites are Not ready/Inconclusive. Disabled or failed Groq means fallback-only assessment.",
        "- No input variants, repairs or IR-07 and later cases are executed.",
    ]))
    save(folder,"source_preflight.json",{
        ...
        "input_interpretation":"Wi-Fi/DNS, VPN, Outlook, printer and remote desktop are named. No single confirmed fault; do not manufacture a correct category label.",
        "confirmed_fault":None,
        "reviewed_approved_KB_families":[{"category":a,"title":b,"body":c} for a,b,c in kb_families],
        "resolved_ticket_title_resolution_families":[{"category":a,"title":b,"resolution":c} for a,b,c in ticket_families],
        "relevance_rule":"Named-topic overlap may make a source related, but does not establish the source's particular diagnosis or corrective procedure for this user.",
    ...
    save(folder,"preconditions.json",{
        "recorded_before_request_at_utc":now(),"case":"IR-06","original_database_main_file_sha256":database_hash,
    ...
    if not all(comparable[key] for key in ("source_files_same_as_IR01","kb_csv_same_as_IR01","ticket_csv_same_as_IR01")):
        save(folder,"execution.json",{"case":"IR-06","status":"Not ready","outcome":"Unassessed","reason":"Source/corpus changed since IR-01; no ticket submitted"})
        print("IR-06 evidence: "+str(folder))
        return 1
...
if __name__=="__main__":
    ...
    if args.worker:
        ...
        allowed=(ROOT/"audit"/"evidence"/"IR-06").resolve()
        if not resolved.is_relative_to(allowed):
            raise SystemExit("Evidence path must be inside audit/evidence/IR-06")
