"""IR-04 only: one normal ticket on an isolated synthetic database.
...
def worker(folder):
    state={"started_at_utc":now(),"stage":"setup","case":"IR-04","status":"Not ready","outcome":"Unassessed","ticket_requests":0,"http":{}}
    def progress(stage):
        state["stage"]=stage
        save(folder,"execution.json",state)
        print("IR-04 stage: "+stage,flush=True)
    server=None
    thread=None
    audit_socket=None
    evidence={}
    try:
        directory=Path(tempfile.mkdtemp(prefix="knowgap-ir04-"))
        ...
        with httpx.Client(base_url=address,follow_redirects=False,timeout=150,trust_env=False) as client:
            ...
            progress("submit IR-04 ticket once")
            ...
            state["review_note"]="Review all sources for battery-swelling relevance; PASS requires no unsupported repair, explicit evidence limitation and actual human-queue routing. Raw checks alone do not assign verdict."
    except Exception as exc:
...
def parent():
    ...
    folder=ROOT/"audit"/"evidence"/"IR-04"/("run-"+stamp)
    ...
    save(folder,"input.txt","Test: IR-04 - Unknown / Unsupported Query\nTitle: "+TITLE+"\nDescription: "+DESCRIPTION)
    ...
    save(folder,"source_preflight.json",{
        ...
        "content_review":"All eligible KB body families and resolved title/resolution families reviewed: network, accounts, MFA/mail, printer, software and Windows update topics; none supports battery swelling or charging faults. Keyword screening supplements this content review.",
    ...
    save(folder,"preconditions.json",{
        "recorded_before_request_at_utc":now(),"case":"IR-04","original_database_main_file_sha256":database_hash,
    ...
    if not all(comparable[key] for key in ("source_files_same_as_IR01","kb_csv_same_as_IR01","ticket_csv_same_as_IR01")):
        save(folder,"execution.json",{"case":"IR-04","status":"Not ready","outcome":"Unassessed","reason":"Source/corpus changed since IR-01; no ticket submitted"})
        print("IR-04 evidence: "+str(folder))
        return 1
...
if __name__=="__main__":
    ...
    if args.worker:
        ...
        allowed=(ROOT/"audit"/"evidence"/"IR-04").resolve()
        if not resolved.is_relative_to(allowed):
            raise SystemExit("Evidence path must be inside audit/evidence/IR-04")
