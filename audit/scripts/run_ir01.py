"""IR-01 only: one normal ticket on an isolated synthetic database.
...
def worker(folder):
    state={"started_at_utc":now(),"stage":"setup","case":"IR-01","status":"Not ready","outcome":"Unassessed","ticket_requests":0,"http":{}}
    def progress(stage):
        state["stage"]=stage
        save(folder,"execution.json",state)
        print("IR-01 stage: "+stage,flush=True)
    server=None
    thread=None
    audit_socket=None
    evidence={}
    try:
        directory=Path(tempfile.mkdtemp(prefix="knowgap-ir01-"))
        ...
        with httpx.Client(base_url=address,follow_redirects=False,timeout=150,trust_env=False) as client:
            ...
            progress("submit IR-01 ticket once")
...
def parent():
    ...
    folder=ROOT/"audit"/"evidence"/"IR-01"/("run-"+stamp)
    ...
    save(folder,"input.txt","Test: IR-01 - Exact Known Issue Retrieval\nTitle: "+TITLE+"\nDescription: "+DESCRIPTION)
    save(folder,"expected_result.md","\n".join([
        ...
        "- Submit exactly one ticket with the saved title and description.",
        "- The top evidence should address connected Wi-Fi with no internet, not merely share a category label.",
    ...
    save(folder,"preconditions.json",{
        "recorded_before_request_at_utc":now(),"case":"IR-01","original_database_main_file_sha256":database_hash,
    ...
    if not backup_verified:
        ...
        print("IR-01 evidence: "+str(folder))
        return 1
...
if __name__=="__main__":
    ...
    if args.worker:
        ...
        allowed=(ROOT/"audit"/"evidence"/"IR-01").resolve()
        if not resolved.is_relative_to(allowed):
            raise SystemExit("Evidence path must be inside audit/evidence/IR-01")
